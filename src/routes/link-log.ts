...
type LinkLogHandler = (context: LinkLogContext) => Promise<Response | null>;

const routes: LinkLogHandler[] = [handleLinkLogSubmissionRoute, handleLinkLogMetadataRoute, handleLinkLogPage];

export async function routeLinkLog(
  path: string,
  request: Request,
  env: Env,
  origin: string,
...
async function handleLinkLogSubmissionRoute({ path, request, env }: LinkLogContext): Promise<Response | null> {
...
async function handleLinkLogMetadataRoute({ path, request, env }: LinkLogContext): Promise<Response | null> {
...
async function handleLinkLogPage({ path, request, env, origin, isPartialRequest }: LinkLogContext): Promise<Response | null> {
  if (path !== "/link-log") {
    return null;
  }
