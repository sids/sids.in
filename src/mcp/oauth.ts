...
import { isBlogScope, OAUTH_AUTHORIZE_ROUTE } from "./config.ts";
...
export function oauthAuthorizationHandler(fallback: FetchHandler): FetchHandler {
  return {
    async fetch(request, env, ctx): Promise<Response> {
      ...
      if (url.pathname !== OAUTH_AUTHORIZE_ROUTE) return fallback.fetch(request, env, ctx);
      ...
      const scope = oauthRequest.scope.filter(isBlogScope);
...
function consentPage(request: AuthRequest, client: ClientInfo, csrf: string): Response {
  ...
  const scopes = request.scope.filter(isBlogScope);
