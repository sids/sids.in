...
export function homeTemplate(page: Page, recentPosts: PostMeta[]): string {
  ...
  <section class="mt-12">
    <h2 class="font-mono text-2xl font-medium mb-4" style="color: var(--text-primary)">
      <a href="/posts" class="nav-link">Recent Posts</a>
    </h2>
    <ul>
      ${postItems}
    </ul>
    <p class="mt-6" style="color: var(--text-secondary)">
      Browse the <a href="/archive" class="nav-link">archive</a> or subscribe to the <a href="/posts/feed.xml" class="nav-link">RSS feed</a>.
    </p>
  </section>` : "";
